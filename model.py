...
class ClusterActorCritic(ActorCritic):
    ...
    def store_transition(self, s1, action, r, s2, alist, intrinsic, weight, feature_names=None, performance=None):
        if type(self.memory) is replay.PERClusterReplay:
            state_value1 = self.critic.forward(s1.reshape(1, -1)).reshape(1)
            state_value2 = self.critic.forward(s2.reshape(1, -1)).reshape(1)
            # print("r:", r)
            # print(state_value1)
            # print(state_value2)
            advantage = abs(r + self.GAMMA * state_value2 - state_value1) + weight * intrinsic
            print("------advantage:", advantage)
            self.memory.store_transition((s1, action, r, s2, alist, feature_names, performance), advantage)
        elif type(self.memory) is replay.RandomClusterReplay:
            self.memory.store_transition((s1, action, r, s2, alist, feature_names, performance))
        else:
            print('replay strategy ERROR!!!!')
    ...
    def learn(self, optimizer):
        ...
        if type(self.memory) is replay.PERClusterReplay:
            weight, s1, a, r, s2, alist = self.memory.sample()
        elif type(self.memory) is replay.RandomClusterReplay:
            s1, a, r, s2, alist = self.memory.sample()
        else:
            print('replay strategy ERROR!!!!')
        ...
        if type(self.memory) is replay.PERClusterReplay:
            ...
            entropy_loss = torch.sum(weight_tensor * torch.FloatTensor(entropy_loss)) / (flag_len + 1)
        elif type(self.memory) is replay.RandomClusterReplay:
            entropy_loss = self.ENT_WEIGHT * torch.mean(torch.cat(entropy_loss, 0))
        else:
            print('replay strategy ERROR!!!!')
...
class OpActorCritic(ActorCritic):
    ...
    def store_transition(self, s1, op, r, s2, intrinsic, weight, feature_names=None, performance=None):
        if type(self.memory) is replay.PEROperationReplay:
            state_value1 = self.get_value(s1)
            state_value2 = self.get_value(s2)
            advantage = abs(r + self.GAMMA * state_value2 - state_value1) + weight * intrinsic
            self.memory.store_transition((s1, op, r, s2, feature_names, performance), advantage)
        elif type(self.memory) is replay.RandomOperationReplay:
            self.memory.store_transition((s1, op, r, s2, feature_names, performance))
        else:
            print('replay strategy ERROR!!!!')
    ...
    def learn(self, optimizer):
        if type(self.memory) is replay.PEROperationReplay:
            weight, s1, op, r, s2 = self.memory.sample()
        elif type(self.memory) is replay.RandomOperationReplay:
        ...
        if type(self.memory) is replay.PEROperationReplay:
            weight_tensor = torch.FloatTensor(weight)
            entropy_loss = torch.mean(weight_tensor * torch.FloatTensor(entropy_loss))
        elif type(self.memory) is replay.RandomOperationReplay:
            entropy_loss = self.ENT_WEIGHT * entropy_loss.mean()
        else:
            print('replay strategy ERROR!!!!')
